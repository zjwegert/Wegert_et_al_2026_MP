...
α_coeff = 3
iter_mod = 1
path = "results/aniso_obstacle_vf$vf/"
mkpath(path)
...
# Level-set function for obstacle regions
V_φ_obs = TestFESpace(model,reffe_scalar)
f_ball((x,y),a,b) = max(abs(x-a),abs(y-b)) - 0.1 + h/2
φh_obstacle = interpolate(x -> min(f_ball(x,0.25,0.5),f_ball(x,0.75,0.5),
  f_ball(x,0.5,0.25),f_ball(x,0.5,0.75)),V_φ_obs)
...
GridapTopOpt.correct_ls!(φh_obstacle)
...
geo_obstacle = DiscreteGeometryFromFEFunction(φh_obstacle,model,name="φ_obstacle")
function compute_geo(φh1,φh2)
  ...
  Ω1 = setdiff(setdiff(setdiff(geo1,geo2),geo_obstacle),geo_diri,name="Ω1")
  Ω3 = union(setdiff(intersect(geo1,geo2),geo_obstacle),geo_diri,name="Ω3")
...
Ωs = EmbeddedCollection(model,φh;compute_cut=false) do φh
  ...
  φ_obstacle = get_free_dof_values(φh_obstacle)
  φ_Ω1orΩ3 = max.(min.(max.(φ1,-φ2),max.(φ1,φ2),φ_diri),-φ_obstacle);
...
writevtk(Ωs.Ω1,path*"Omega1_0")
writevtk(Ωs.Ω3,path*"Omega3_0")
writevtk(Ωs.Γg1,path*"Γg1_0")
writevtk(Ωs.Γg3,path*"Γg3_0")
writevtk(Ωs.Γ13,path*"Gamma13_0")
